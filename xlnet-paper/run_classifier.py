...
# task specific
flags.DEFINE_string("task_name", default="paper", help="Task name")
...
class PaperProcessor:
    ...
    def get_train_examples(self, data_dir, shuffle=True):
        examples = []
        items = self._read_jsonl(os.path.join(
            data_dir, "train.jsonl"))
        for item in items:
            text_a = [preprocess_text(
...
def main(_):
    ...
    if FLAGS.do_eval or FLAGS.do_predict:
        if FLAGS.eval_split == "dev":
            ...
            eval_examples = processor.get_train_examples(FLAGS.data_dir, shuffle=False)
        else:
