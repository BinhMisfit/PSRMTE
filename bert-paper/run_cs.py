...
class PaperProcessor:
    def __init__(self, features=["abstract"]):
        self.features = features

    def get_train_examples(self, data_dir):
        examples = []
        items = self._read_jsonl(os.path.join(
            data_dir, "cs-train.jsonl"))
        for item in items:
            text_a = [tokenization.convert_to_unicode(
                item[feature]) for feature in self.features]
            text_a = [preprocess_abstract(text) if feature == "abstract" else text
                      for text, feature in zip(text_a, self.features)]
            text_a = " ".join(text_a)
    ...
    def get_dev_examples(self, data_dir):
        ...
        for item in items:
            ...
            #           for text, feature in zip(text_a, self.features)]
            text_a = " ".join(text_a)
            label = tokenization.convert_to_unicode(item['journal']).lower()
            guid = item['id']
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=None, label=label))
        return examples
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        best_checkpoint, best_eval = None, None
        checkpoints = sorted([f for f in tf.gfile.ListDirectory(FLAGS.output_dir)
                              if f[-6:] == ".index" and int(f[11:-6]) > 0], key=lambda x: int(x[11:-6]))
        output_eval_file = os.path.join(FLAGS.output_dir, "eval_results.txt")
        with tf.gfile.GFile(output_eval_file, "w") as writer:
            for checkpoint in checkpoints:
                checkpoint = os.path.join(FLAGS.output_dir, checkpoint)[:-6]
                result = estimator.evaluate(input_fn=eval_input_fn,
                                            steps=eval_steps,
                                            checkpoint_path=checkpoint)
                if best_eval is None or best_eval < result['eval_accuracy']:
                    best_eval = result['eval_accuracy']
            ...
            tf.logging.info(
                "***** Eval results *****: {}".format(best_checkpoint))
            tf.logging.info("eval_accuracy: {}".format(best_eval))
