...
class ModelRunner(object):
  ...
  def evaluate_task(self, task, split="dev", return_results=True):
    ...
    checkpoints = sorted([f for f in tf.gfile.ListDirectory(self._config.model_dir)
                              if f[-6:] == ".index"], key=lambda x: int(x[11:-6]))
    checkpoints = [os.path.join(self._config.model_dir, checkpoint[:-6]) for checkpoint in checkpoints]
    best_scores = None
    best_scorer = None
    ...
    for checkpoint in checkpoints:
      ...
      if return_results:
        utils.log(task.name + ": " + " - ".join("{}: {}".format(k, v) for k, v in scores.items()))
        utils.log()
        if key is None or best_scores is None or scores[key] > best_scores[key]:
          best_scores = scores
      else:
        if key is None or best_scores is None or scores[key] > best_scores[key]:
          best_scores = scores
          best_scorer = scorer
    if return_results:
      utils.log("eval_results " + task.name + ": " + " - ".join("{}: {}".format(k, v) for k, v in best_scores.items()))
      return best_scores
    else:
      return best_scorer

  def evaluate_task_test(self, task, checkpoint, split="test", return_results=True):
    """Evaluate the current model."""
    ...
    if return_results:
      utils.log("test_results " + task.name + ": " + " - ".join("{}: {}".format(k, v) for k, v in scores.items()))
      return scores
    else:
      return scorer

  def predict(self, task, checkpoint_path, split):
...
def run_finetuning(config: configure_finetuning.FinetuningConfig):
  ...
  while config.num_trials < 0 or trial <= config.num_trials:
    ...
    if config.do_eval:
      ...
      if config.do_test:
        for task in tasks:
          test_score = model_runner.evaluate_task_test(task, results[-1][task.name]['checkpoint_path'])
    ...
    if config.do_predict:
      if "dev" in config.predict_split:
        results = model_runner.predict(tasks[0], config.predict_checkpoint_path, "dev")
        import pickle
        with open("predict_dev.pickle", "bw") as outfile:
          pickle.dump(results, outfile)

      if "train" in config.predict_split:
        results = model_runner.predict(tasks[0], config.predict_checkpoint_path, "train")
        import pickle
        with open("predict_train.pickle", "bw") as outfile:
          pickle.dump(results, outfile)

      if "test" in config.predict_split:
        results = model_runner.predict(tasks[0], config.predict_checkpoint_path, "test")
        import pickle
...
def main():
  ...
  parser.add_argument("--data-dir", default="Paper-Springer", required=True,
                      help="Location of data files (model weights, etc).")
  parser.add_argument("--model-name", default="test", required=True,
...
if __name__ == "__main__":
  import sys
  # sys.argv = ["run_finetuning.py", "--data-dir=Paper-Springer/", "--model-name=test", "--hparams=paper_config.json"]
  print(sys.argv)
  main()
